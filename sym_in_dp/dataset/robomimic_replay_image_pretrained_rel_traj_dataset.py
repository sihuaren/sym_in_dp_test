...
from sym_in_dp.dataset.base_dataset import BaseImageDataset, LinearNormalizer
from sym_in_dp.model.common.normalizer import LinearNormalizer, SingleFieldLinearNormalizer
from sym_in_dp.model.common.rotation_transformer import RotationTransformer
from sym_in_dp.codecs.imagecodecs_numcodecs import register_codecs, Jpeg2k
from sym_in_dp.common.replay_buffer import ReplayBuffer
from sym_in_dp.common.sampler import SequenceSampler, get_val_mask
from sym_in_dp.common.normalize_util import (
...
class RobomimicReplayImagePretrainedRelTrajDataset(BaseImageDataset):
    def __init__(self,
                 shape_meta: dict,
                 dataset_path: str,
                 horizon=1,
    ...
    def get_normalizer(self, mode='limits', **kwargs) -> LinearNormalizer:
        self.ws_center = np.array([0., 0.])
        # data = {
        #     'point_cloud': self.replay_buffer['point_cloud'],
        # }
        normalizer = LinearNormalizer()
        # stat = array_to_stats(
        #     self.replay_buffer['point_cloud'][:].reshape(-1, self.replay_buffer['point_cloud'].shape[-1]))
            ...
            if stat['mean'].shape[-1] > 10:
                ...
                for i in range(self.replay_buffer.meta.episode_ends.shape[0]):
                    ...
                    for horizon in range(self.horizon):
                        max_horizon_steps = self.replay_buffer['action'][start + horizon:end, :3] - self.replay_buffer[
                                                                                                        'robot0_eef_pos'][
                                                                                                    start:end - horizon]
