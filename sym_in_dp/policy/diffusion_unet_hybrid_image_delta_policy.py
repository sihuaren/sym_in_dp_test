...
from sym_in_dp.common.robomimic_config_util import get_robomimic_config
...
from sym_in_dp.common.pytorch_util import dict_apply, replace_submodules
...
class DiffusionUnetHybridImageDeltaPolicy(BaseImagePolicy):
    def __init__(self, 
            shape_meta: dict,
    ...
    def predict_action(self, obs_dict: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        del_T = torch.eye(4).repeat(B, T, 1, 1).to(device)
        del_T[:, :, :3, :3] = rot
        del_T[:, :, :3, 3] = xyz

        cur_T = torch.eye(4).repeat(B, 1, 1, 1).to(device)
        ...
        abs_T = del_T.clone()
        abs_T[:, 0] = torch.matmul(cur_T[:, 0], del_T[:, 0]) 
        for i in range(1, T):
            abs_T[:, i] = torch.matmul(abs_T[:, i-1], del_T[:, i])
    ...
    def compute_loss(self, batch):
        ...
        del_T = torch.eye(4).repeat(batch['action'].shape[0], batch['action'].shape[1], 1, 1).to(self.device)
        
        del_T[:, 0] = cur_T.inverse()[:, 0] @ abs_T[:, 0]
        for i in range(1, batch['action'].shape[1]):
            del_T[:, i] = abs_T[:, i-1].inverse() @ abs_T[:, i]
        # for i in range(1, batch['action'].shape[1]):
        #     del_T[:, i] = rel_T[:, i-1].inverse() @ abs_T[:, i]

        del_xyz = del_T[:, :, :3, 3]
        del_6d = self.sixd_to_mat.inverse(del_T[:, :, :3, :3])
        del_action = torch.cat([del_xyz, del_6d, batch['action'][:, :, 9:]], dim=-1)

        batch['action'] = del_action
